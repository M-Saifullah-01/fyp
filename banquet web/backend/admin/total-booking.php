...
if ($filter) {
  $select = "SELECT * FROM total_booking  WHERE person_name LIKE'%$filter%' OR total_booking.name  LIKE '%filter%' OR Booking_No LIKE'%$filter%' OR cnic LIKE'$filter'";
  $select_query = mysqli_query($connection, $select);
} else {
  // $select = "SELECT Booking_No, person_name, phone_number, cnic, booking_date, Booking_time, address, add_pakage.name as pkg_name ,people, add_menu.headprice as headprice, add_menu.name as menu_name,  user_id FROM total_booking INNER JOIN add_pakage ON total_booking.pkg_id = add_pakage.pkg_id INNER JOIN add_menu ON total_booking.menu_id = add_menu.menu_id";
  $select = "SELECT Booking_No, person_name, phone_number, cnic, booking_date, Booking_time, address, total_booking.name as hall_name, hall_no, capacity ,total_booking.headprice as headprice,  add_menu.name as menu_name, add_menu.menu_id as menu_id, user_id FROM total_booking INNER JOIN  add_menu ON total_booking.menu_id = add_menu.menu_id";
  $select_query = mysqli_query($connection, $select);
...
<head>
  ...
  <!--Link with css-->
  <link rel="stylesheet" href="assets/css/total-booking.css">
  <!--font awesome link-->
      ...
      <div class="card ">
        <div class="card-header d-flex align-items-center justify-content-between">
          <span class="d-flex align-items-center ml-3">
            ...
            <h3 class="ml-5">Total Bookings Request</h3>
          ...
          <form action="" class="d-flex align-items-center">
            ...
            <a href="total-booking.php" class="btn btn-success ml-1"><i class="fa-solid fa-house"></i></a>
          </form>
        </div>
