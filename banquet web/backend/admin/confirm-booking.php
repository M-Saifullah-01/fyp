...
if ($filter) {
  $select = "SELECT * FROM confirm_booking  WHERE person_name LIKE'%$filter%' OR Booking_No LIKE'%$filter%' OR cnic LIKE'$filter' OR pkg_name LIKE'%$filter%'";
  $select_query = mysqli_query($connection, $select);
} else {
  $select = "SELECT * FROM confirm_booking";
  $select_query = mysqli_query($connection, $select);
}
      ...
      <div class="card ">
        <div class="card-header d-flex align-items-center justify-content-between">
          <span class="d-flex align-items-center ml-3">
            ...
            <h3 class="ml-5">Confirmed Bookings</h3>
          </span>
          <form action="" class="d-flex align-items-center">
            <input type="text" name="filter" class="form-control">
            <button class="btn btn-primary ml-1" type="submit"><i class="fa-solid fa-magnifying-glass"></i></button>
            <a href="confirm-booking.php" class="btn btn-success ml-1"><i class="fa-solid fa-house"></i></a>
          </form>
        </div>
